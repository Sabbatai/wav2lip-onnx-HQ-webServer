...
def main():
  ...
  # ffmpeg preset for HQ processing	
    ...
    # crop final:
		# crop final - auto-use full frame:
		h, w = orig_frame.shape[:-1]
		roi = (0, 0, w, h)  # Use full frame automatically
		cv2.destroyAllWindows()
    ...
    # read frames and crop roi:
      ...
      # crop first frame:
        ...
        # select_specific_face:
				# Auto-detect primary face (no ROI selection):
				bboxes, kpss = detector.detect(cropped_roi, input_size=(320,320), det_thresh=0.3)
				assert len(kpss) != 0, "No face detected"
				target_face, mat = get_cropped_head_256(cropped_roi, kpss[0], size=256, scale=1.0)
				target_face = cv2.resize(target_face, (112, 112))
				target_id = recognition(target_face)[0].flatten()
				orig_h, orig_w = cropped_roi.shape[:-1]
