...
# Configuration
TRT_ENGINE_PATH = "checkpoints/wav2lip_gan_batch.trt"
IMG_SIZE = 96
FPS = 15
BATCH_SIZE = 8  # Process 4 frames at once
...
def infer_trt(img_batch, mel_batch):
    ...
    # Set input shapes explicitly for dynamic batch
    ...
    # Allocate output tensor with correct batch size
    output_tensor = torch.empty((actual_batch_size, 3, IMG_SIZE, IMG_SIZE), dtype=torch.float32, device='cuda')
...
def datagen(frames, mels, static=False, batch_size=1):
    ...
    for i, m in enumerate(mels):
        ...
        # Yield when batch is full or at the end
        if len(img_batch) == batch_size or i == len(mels) - 1:
...
async def generate_video(request: VideoRequest):
    ...
    try:
        ...
        # Generate video
        gen = datagen(sub_faces, mel_chunks, static=False, batch_size=BATCH_SIZE)
        ...
        fc = 0
        frame_count = 0
        total_batches = (len(mel_chunks) + BATCH_SIZE - 1) // BATCH_SIZE
        for i, (img_batch, mel_batch, frames) in enumerate(tqdm(gen, total=total_batches, desc="TensorRT inference")):
            # Run TensorRT inference on batch
            preds = infer_trt(img_batch, mel_batch)
